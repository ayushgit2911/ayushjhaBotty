...
async def load_reload(event):
    ...
    try:
        ...
        msg = await event.respond(f"muted Successfully (re)load pligon {shortname}")
...
async def remove(event):
    ...
    if shortname == "_core":
        ...
        msg = await event.respond(f"Pligon {shortname} is not load...")
    await asyncio.sleep(DELETE_TIMEOUT)
    await msg.delete()
...
async def send_plug_in(event):
    ...
    await event.edit("thik hai is pligon ko upload kar diya {} in {} seconds".format(input_str, time_taken_in_ms))
    await asyncio.sleep(DELETE_TIMEOUT)
    await event.delete()
    ...
    if event.reply_to_msg_id:
        try:
            ...
            if "(" not in downloaded_file_name:
                ...
                await event.edit("oye oye, Instulled Pligon `{}`".format(os.path.basename(downloaded_file_name)))
            else:
                os.remove(downloaded_file_name)
                await event.edit("oh! Hoh, pligon instull na hove.")
        except Exception as e:  # pylint:disable=C0103,W0703
